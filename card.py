...
PALETTES = {
    ...
    "autumn": {
        "background": "#211B19", "card": "#3B3029", "white": "#FFF2DE", "muted": "#C8B9A7",
        "accent": "#D69A5E",
...
def draw_leaf(draw: ImageDraw.ImageDraw, x: int, y: int, size: int,
    ...
    points = [(0, -1), (0.22, -0.66), (0.41, -0.79), (0.36, -0.38),
              (0.66, -0.30), (0.38, -0.06), (0.54, 0.23), (0.23, 0.27),
              (0, 0.85), (-0.23, 0.27), (-0.54, 0.23), (-0.38, -0.06),
              (-0.66, -0.30), (-0.36, -0.38), (-0.41, -0.79), (-0.22, -0.66)]
    c, s = math.cos(angle), math.sin(angle)
    transformed = [(x + int((px * c - py * s) * size),
                    y + int((px * s + py * c) * size)) for px, py in points]
    draw.polygon(transformed, fill=color)
    draw.line((x + int(s * size), y - int(c * size),
               x - int(s * size), y + int(c * size)), fill="#E6B985", width=max(1, size // 20))
...
def render_card(group_name: str, date: str, blocks: list[dict], slot_count: int,
                theme: str = "classic") -> bytes:
    """Вернуть PNG в памяти. blocks содержат уже подготовленные данные занятий."""
    colors = palette(theme)
    BACKGROUND, CARD, WHITE, MUTED, CYAN = (colors[key] for key in
                                             ("background", "card", "white", "muted", "accent"))
    TYPE_COLORS, TYPE_TINTS = colors["types"], colors["tints"]
    ...
    draw.rounded_rectangle((772, 74, 912, 122), radius=18, fill=colors["header"])
    group_font = next((font(size) for size in (24, 22, 20, 18, 16)
                       if text_width(draw, group_name, font(size)) <= 124), font(16))
    group_width = text_width(draw, group_name, group_font)
    draw.text((842 - group_width / 2, 83), group_name, font=group_font, fill=WHITE)
    ...
    draw.rounded_rectangle((722, 186, 912, 238), radius=22, fill=colors["badge"])
    ...
    for row in layout:
        ...
        if block.get("window"):
            draw.rounded_rectangle((x1, y1, x2, y2), radius=24,
                                   fill=colors["window"], outline=colors["window_outline"], width=2)
            pair_text = (f"{block['pairs'][0]}-я пара" if len(block["pairs"]) == 1
                         else f"Пары {block['pairs'][0]}–{block['pairs'][-1]}")
            draw.text((78, y1 + 21), "ОКНО", font=medium_26, fill=colors["window_text"])
        ...
        draw.rounded_rectangle((886 - chip_width, y1 + 28, 886, y1 + 70), radius=16,
                               fill=TYPE_TINTS.get(block["type"], colors["badge"]))
        ...
        if row["room_lines"]:
            ...
            draw.rounded_rectangle((648, room_top, 900, room_top + row["room_height"]),
                                   radius=18, fill=colors["room"], outline=accent, width=3)
    ...
    footer_y = height - 55
    draw.text((48, footer_y), "Источник: ДГТУ", font=regular_22, fill=colors["footer"])
    if theme == "autumn":
        draw_leaf(draw, 853, footer_y + 13, 22, -0.5, "#A85F38")
        draw_leaf(draw, 898, footer_y + 5, 16, 0.6, "#D4914C")
    output = io.BytesIO()
    image.save(output, format="PNG", optimize=True)
    return output.getvalue()


def render_week_card(group_name: str, monday: str, days: list[dict],
                     theme: str = "classic") -> bytes:
    ...
    draw.rounded_rectangle((772, 73, 912, 122), radius=18, fill=colors["header"])
    group_font = next((font(size) for size in (25, 23, 21, 19, 17)
                       if text_width(draw, group_name, font(size)) <= 124), font(17))
    draw.text((842 - text_width(draw, group_name, group_font) / 2, 83),
              group_name, font=group_font, fill=WHITE)
    ...
    for section in layouts:
        day = section["day"]
        sy = section["y"]
        draw.rounded_rectangle((42, sy, 918, sy + section["height"]),
                               radius=26, fill=colors["section"])
        ...
        slots = day["slots"]
        form = ("пара" if slots % 10 == 1 and slots % 100 != 11 else
                "пары" if slots % 10 in (2, 3, 4) and slots % 100 not in (12, 13, 14) else "пар")
        count = f"{slots} {form}"
        ...
        for row in section["rows"]:
            ...
            h = row["height"]
            if block.get("window"):
                draw.rounded_rectangle((62, ry, 898, ry + h), radius=16, fill=colors["week_window"])
                pairs = block["pairs"]
                label = f"{pairs[0]}-я пара" if len(pairs) == 1 else f"пары {pairs[0]}–{pairs[-1]}"
                draw.text((82, ry + 19), label, font=medium, fill=MUTED)
                draw.text((300, ry + 19), "ОКНО", font=medium_bold, fill=colors["window_text"])
                time_text = f"{block['start']}–{block['end']}"
    ...
    if theme == "autumn":
        draw_leaf(draw, 853, height - 44, 22, -0.5, "#A85F38")
        draw_leaf(draw, 898, height - 52, 16, 0.6, "#D4914C")
    output = io.BytesIO()
    image.save(output, format="PNG", optimize=True)
    return output.getvalue()
